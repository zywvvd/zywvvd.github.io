...
1. 图像异常检测技术由来
2. 异常检测技术方向, 分析各个方法优缺点以及限制条件
3. 常用数据集
4. 评价指标
5. 代表性工作介绍以及相关资料索引
6. 根据调研信息总结当前图像异常检测技术现状, 展望工作中可能用到的场景
...
3. **基于数据的决策**：在其核心, 异常检测是一种数据驱动的决策过程. 它依赖于数据分析来揭示不寻常或罕见的模式, 而不是仅仅依赖于预设的假设或规则. 
4. **动态和自适应**：异常检测系统通常需要具有动态性和自适应性, 因为随着时间的推移, 数据的性质和“正常”行为的定义可能会发生变化. 因此, 这些系统需要能够适应新的数据模式和变化的环境条件. 
5. **平衡灵敏度和特异性**：异常检测的一个关键挑战是平衡灵敏度（检测到尽可能多的真实异常）和特异性（减少错误报警）. 这通常涉及在避免漏检和误报之间寻找最佳平衡点. 
6. **多学科融合**：异常检测融合了统计学、机器学习、数据挖掘、计算机视觉等多个学科的理论和方法. 这种多学科融合使得异常检测能够应用于各种领域和场景. 
...
异常检测的本质是将数据映射到一个可度量的空间中, 根据训练样本找到“正常”数据在该空间中的区域（可能是随着模型固定的, 可能是随着测试数据确定的）. 执行异常检测时查看测试数据是否落入此“正常”范围内. 
...
5. **监督学习的标签数据**：在监督学习的场景中, 正常的定义可能来自于标记的训练数据. 通过训练模型识别这些已标记为正常的数据点, 可以帮助系统理解并识别正常情况. 
...
采用 POFT 来消除背景区域. POFT 在对图像进行傅里叶变换 之后, 抛弃了幅度谱的信息而仅采用其相位谱信息来进行逆傅里叶变换:
$$
\begin{array}{c}
...
- 缺点
  ...
  3. **异常检测的特殊性**：异常检测通常需要模型能够识别出非常细微的差异, 这可能对教师网络和学生网络都是一个挑战. 因此, 这种方法在处理高度复杂或细微的异常时可能效果不佳. 
...
| 监督MSE  | $\max \{|x_{abnormal}-x_{abnormal} ^{\prime} |^2-| x_{normal} -x_{normal} ^ {\prime}|^2\} $ | 如果存在异常数据可以用该方法重构正常样本的同时破坏异常样本的重建 |
...
2. **不需要大量标签数据**：这种方法不依赖于大量的标记异常样本，适用于那些难以获取标记数据的场景。
3. **发现异常模式**：这种方法不仅可以检测单个异常点，还可以识别异常的集群或模式，这对于理解数据中的潜在问题非常重要。
...
| 适用场景           | 正常数据量很多，异常定义不清晰或数据量很少，性能要求不高时可以使用该方法 |      |
...
可以理解为在特征空间上有监督的一分类训练，手动创建正常和异常数据的边界交给网络去学习。
...
在训练过程中, 正常或异常图像被送入重构网络, 使用重构损失函数来引导重构网络的训练. 最终, 重构网络可以以与原始正常图像类似的方式还原重构图像. 在推断阶段, 比较模型将原始图像与重构图像进行比较以生成预测. 与各种特征嵌入方法不同, 大多数基于重构的方法只在重构网络的构建方面有所不同. 
...
| Area Under the Receiver Operating Characteristic curve (AU-ROC) | $\int^1_0(TPR)d(FRR)$                                   | ↑    |
...
| ------------------- | ------------------------------------------------------------ | ---- |
| 方法名称            | EfficientAD                                                  |      |
| 论文题目            | EfficientAD: Accurate Visual Anomaly Detection at Millisecond-Level Latencies |      |
...
$$
作者认为：这种惩罚会阻碍学生将对老师的模仿推广到分布外的图像. （**为啥？？**）
...
1. https://blog.csdn.net/qq_37337494/article/details/116596799
2. https://arxiv.org/pdf/2305.15956v2.pdf
3. https://github.com/arimousa/DDAD
4. https://zhuanlan.zhihu.com/p/364914260
5. https://zhuanlan.zhihu.com/p/371160299
6. https://zhuanlan.zhihu.com/p/531151430
7. https://blog.csdn.net/qq_36560894/article/details/119608867
8. https://blog.csdn.net/qq_36560894/article/details/121509629
9. https://zhuanlan.zhihu.com/p/97482962
10. https://yinglinzheng.netlify.app/diffusion-model-tutorial/
