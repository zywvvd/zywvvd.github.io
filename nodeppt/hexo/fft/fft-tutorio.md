...
<slide :class="bg-white fullscreen aligncenter">
...
- 证明：需要证明 cos sin 两两之间的积分为零，因为道理相同，这里以 cos cos 为例： {.text-intro}
...
- 一维傅里叶变换是将一个一维的信号分解成若干个复指数波$e^{j w x}$。

- 而由于$e^{j w x}=\cos (w x)+i \sin (w x)$，所以可以将每一个复指数波$e^{j w x}$都视为是 `余弦波` $+\mathrm{j}\times$ `正弦波` 的组合。
...
- 从公式也可以看到，二维傅里叶变换就是将图像与每个不同频率的不同方向的复平面波做内积，也就是一个求在基 $e^{-j 2 \pi(u x+v y)}$上的投影的过程。
...
- 运用频域乘法计算时
  - 空域转换为频域，可以使用 FFT，时间复杂度为$O(N^2logN)$
  - 乘法运算复杂度为$O(N^2)$
  - 因此复杂度为$O(N^2logN)$
...
![](https://101.43.39.125/HexoFiles/vvd_file_mt/202211240941478.jpg)
...
![](https://101.43.39.125/HexoFiles/vvd_file_mt/202211240951854.jpg)
...
- 相位相关和互相关的结果一个是冲击信号，一个是相关度计算的结果，在实际应用中相位相关在处理位移搜索时表现也更加鲁棒。
