...
- 在频域中，基是{%raw%} $ \left\{e^{j w x}\right\} $  {%endraw%}，而且这组基是正交基（基于傅里叶级数）
...
- 从公式也可以看到，二维傅里叶变换就是将图像与每个不同频率的不同方向的复平面波做内积，也就是一个求在基 {%raw%} $ \left\{e^{-j 2 \pi(u x+v y)}\right\} $ {%endraw%}上的投影的过程。
...
- 因此可以采用使用互相关计算出的位置，定位后计算两幅图像的相关度，结合鲁棒性和可解释性给出结果。